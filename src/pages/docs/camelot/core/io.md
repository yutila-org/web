...
- **Why it was designed that way**: OS APIs return disparate error types and leak file descriptors on panic.
- **Problems it solves**: Unchecked errors, platform-specific code and untracked buffer allocations.
...
- **Why it was designed that way**: `asprintf` and `vasprintf` bypass custom allocators and generate double-free hazards.
- **Problems it solves**: Buffer overflows from `strcpy` and unmanaged pointers from `asprintf`.
...
### Interop Constraints (Compiler-enforced and Convention-only)
